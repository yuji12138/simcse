...
class SimcseModel(nn.Module):
    ...
    def __init__(self, pretrained_model, pooling, dropout=0.3):
        ...
        self.cls_loss = True
        self.cls_number = 1000000

        self.fc = nn.Sequential(nn.Linear(768, self.cls_number),nn.LayerNorm(self.cls_number),nn.GELU())

    def forward(self, input_ids, attention_mask, token_type_ids):
        out = self.bert(input_ids, attention_mask, token_type_ids, output_hidden_states=True, return_dict=True)
        # return out[1]
        if self.pooling == 'cls':
            if self.cls_loss and self.training:
                cls_fea = out.last_hidden_state[:, 0]# [batch, 768]
                pred_cls = self.fc(cls_fea)
                return cls_fea, pred_cls
            else:
