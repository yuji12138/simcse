...
def train(model, train_loader, dev_loader, optimizer, args):
    ...
    for epoch in range(args.epochs):
        for batch_idx, data in enumerate(tqdm(train_loader)):
            ...
            out, cls_pred = model(input_ids, attention_mask, token_type_ids)
            ...
            if step % args.eval_step == 0:
                ...
                print('loss:{}, corrcoef: {} in step {} epoch {}'.format(loss, corrcoef, step, epoch))
                ...
                if best < corrcoef:
                    ...
                    print('higher corrcoef: {} in step {} epoch {}, save model'.format(best, step, epoch))
...
def load_train_data_unsupervised(tokenizer, args):
    ...
    if os.path.exists(train_file_cache) and not args.overwrite_cache:
        with open(train_file_cache, 'rb') as f:
            ...
            print("len of train data:{}".format(len(feature_list)))
            return feature_list
    feature_list = []
...
#         lines = lines[:6400]
        ...
        for line in tqdm(lines):
            ...
            feature['idx'] = torch.LongTensor([idx,idx])
            idx += 1
...
def load_train_data_supervised(tokenizer, args):
    ...
    获取NLI监督训练语料
    """
    print('loading supervised train data')
    output_path = os.path.dirname(args.output_path)
    ...
    print("len of train data:{}".format(len(df)))
...
def load_eval_data(tokenizer, args, mode):
    ...
    if os.path.exists(eval_file_cache) and not args.overwrite_cache:
        with open(eval_file_cache, 'rb') as f:
            feature_list = pickle.load(f)
            print("len of {} data:{}".format(mode, len(feature_list)))
            return feature_list
    ...
    with open(eval_file, 'r', encoding='utf8') as f:
        lines = f.readlines()
        print("len of {} data:{}".format(mode, len(lines)))
        for line in tqdm(lines):
...
def main(args):
    ...
    if args.do_train:
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=args.batch_size_train, shuffle=True,drop_last=True,
                                      num_workers=args.num_workers)
        dev_data = load_eval_data(tokenizer, args, 'dev')
        dev_dataset = TestDataset(dev_data, tokenizer, max_len=args.max_len)
        dev_dataloader = DataLoader(dev_dataset, batch_size=args.batch_size_eval, shuffle=True,drop_last=True,
                                    num_workers=args.num_workers)
    ...
    if args.do_predict:
        ...
        test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size_eval, shuffle=True,drop_last=True,
                                     num_workers=args.num_workers)
